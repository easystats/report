...
test_that("report.htest", {
  ...
  # testthat::expect_equal(as.list(r)$Difference, -2.786, tol = 0.01)
  r <- report(t.test(iris$Sepal.Width, iris$Sepal.Length))
  # testthat::expect_equal(as.list(r)$Difference, -2.786, tol = 0.01)
  r <- report(t.test(mtcars$mpg ~ mtcars$vs))
  # testthat::expect_equal(as.list(r)$Difference, -7.9404, tol = 0.01)
  r <- report(t.test(iris$Sepal.Width, mu = 1))
  # testthat::expect_equal(as.list(r)$Difference, 2.057, tol = 0.01)
})
