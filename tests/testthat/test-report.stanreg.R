...
test_that("report.stanreg_lm", {
  ...
  testthat::expect_equal(r$values$parameters$wt$Median, -3.19, tol = 0.2)
...
test_that("report.stanreg_glm", {
  ...
  testthat::expect_equal(r$values$parameters$wt$Median, 0.799, tol = 0.2)
  testthat::expect_equal(ncol(to_table(r)), 8)

  r <- report(circus::download_model("stanreg_glm_2"))
  testthat::expect_equal(r$values$parameters$wt$Median, 0.987, tol = 0.2)
  testthat::expect_equal(ncol(to_table(r)), 7)
})
...
test_that("report.stanreg_lmer", {
  ...
  testthat::expect_equal(r$values$parameters$cyl$Median, 0.4062, tol = 0.2)
  testthat::expect_equal(ncol(to_table(r)), 8)
