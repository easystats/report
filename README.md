...
```
    ...
    # to qsec = 0 and wt = 0, is at 19.66 (95% CI [8.31, 30.01]). Within this model:
    # 
    #   - The effect of qsec (Median = 0.93, 95% CI [0.41, 1.52]) has a 99.95% probability of being positive (> 0),
    # 98.50% of being significant (> 0.30), and 0.05% of being large (> 1.81). The estimation successfuly converged
    # (Rhat = 1.000) and the indices are reliable (ESS = 1914)
    #   - The effect of wt (Median = -5.06, 95% CI [-5.97, -4.02]) has a 100.00% probability of being negative (< 0),
    # 100.00% of being significant (< -0.30), and 100.00% of being large (< -1.81). The estimation successfuly
    # converged (Rhat = 0.999) and the indices are reliable (ESS = 1980)
    # 
