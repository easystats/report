...
report.lavaan <- function(
  ...
  if (length(c(ci)) > 1) {
    warning(
      paste0(
        "report does not support multiple `ci` values yet. Using ci = ",
        ci[1]
      ),
      "."
    )
  ...
  performance <- performance::model_performance(
    model,
  ...
  text <- paste0(
    ...
    text_parameters(
      model,
      parameters = params[names(params)[!names(params) %in% c("SE")]]
    )
  )
  text_full <- paste0(
